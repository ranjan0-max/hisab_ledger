...
<body>
    ...
    @isset($renewalWarning)
        <div class="modal fade" id="renewalWarningModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="renewalWarningTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-warning shadow-lg">
                    <div class="modal-header bg-warning-subtle border-warning-subtle">
                        <h5 class="modal-title fw-bold text-warning-emphasis" id="renewalWarningTitle">
                            <i class="bi bi-exclamation-triangle-fill me-2"></i>Server Renewal Reminder
                        </h5>
                    </div>
                    <div class="modal-body text-center py-4">
                        <i class="bi bi-calendar-event text-warning display-4"></i>
                        <h5 class="fw-bold mt-3">Server renewal payment date is approaching.</h5>
                        <p class="text-muted mb-1">Payment is due on <strong>{{ $renewalWarning['dueDate']->format('d M Y') }}</strong>.</p>
                        <p class="mb-0"><strong>{{ $renewalWarning['daysRemaining'] }} days remaining</strong></p>
                    </div>
                    <div class="modal-footer justify-content-center">
                        <form method="POST" action="{{ route('renewal.warning.dismiss') }}">
                            @csrf
                            <button type="submit" class="btn btn-warning px-4">Close Reminder</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            @isset($renewalWarning)
                bootstrap.Modal.getOrCreateInstance(document.getElementById('renewalWarningModal')).show();
            @endisset
