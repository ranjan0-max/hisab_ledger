@php
    $isCustomer = ($type ?? 'REGULAR_CUSTOMER') === 'REGULAR_CUSTOMER';
    $titleText = $isCustomer ? 'CUSTOMERS DIRECTORY' : 'SUPPLIERS DIRECTORY';
    $subTitleText = $isCustomer ? 'Customer Directory & Balance Report' : 'Supplier Directory & Balance Report';
    $nameHeader = $isCustomer ? 'Customer Name' : 'Supplier Name';
    $emptyText = $isCustomer ? 'No customers found.' : 'No suppliers found.';
    $footerText = $isCustomer ? 'Total Customers Listed' : 'Total Suppliers Listed';
@endphp
<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title>{{ $isCustomer ? 'Customer Directory Report' : 'Supplier Directory Report' }}</title>
    <style>
        @page {
            margin: 25px 30px;
        }
...
<body>
            ...
            <td>
                ...
                <div class="text-muted" style="font-size: 9px;">
                    @if(!empty($inactiveMonths))
                        Inactive {{ $isCustomer ? 'Customers' : 'Suppliers' }} Report (No tx in last {{ $inactiveMonths }} {{ $inactiveMonths == 1 ? 'month' : 'months' }} + Pending Balance)
                    @else
                        {{ $subTitleText }}
                    @endif
                </div>
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th style="width: 25%; text-align: left;">{{ $nameHeader }}</th>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="5" class="text-center text-muted" style="padding: 15px;">{{ $emptyText }}</td>
                </tr>
            @endforelse
        </tbody>
    </table>
    ...
    <div class="footer">
        {{ $footerText }}: {{ count($exportContacts) }} | Generated by {{ config('app.name', 'Hisab Ledger') }}
    </div>
</body>
</html>
