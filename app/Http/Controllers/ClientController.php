...
class ClientController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($user->isSuperAdmin()) {
            $validated['next_renewal_date'] = $this->nextRenewalDateForMonth((int) $validated['renewal_month']);
        }

        unset($validated['renewal_month']);
    ...
    public function update(Request $request, Client $client)
    {
        ...
        if ($user->isSuperAdmin()) {
            $renewalMonth = (int) $validated['renewal_month'];
            $currentRenewalMonth = $client->next_renewal_date?->month;

            if (!$client->next_renewal_date || $currentRenewalMonth !== $renewalMonth) {
                $validated['next_renewal_date'] = $this->nextRenewalDateForMonth($renewalMonth);
            }
        }
    ...
    private function nextRenewalDateForMonth(int $month): string
    {
        $today = CarbonImmutable::today('Asia/Kolkata');
        $renewalDate = $today->setMonth($month)->startOfMonth();

        if ($renewalDate->lte($today)) {
            $renewalDate = $renewalDate->addYear();
        }
